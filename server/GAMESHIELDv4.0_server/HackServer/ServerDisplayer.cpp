...
CServerDisplayer::CServerDisplayer() // OK
{
	...
	this->m_brush[1] = CreateSolidBrush(RGB(110, 240, 120)); //Color de fondo modo de activo
	this->m_brush[2] = CreateSolidBrush(RGB(255, 255, 0)); //Color de fondo Franja principal (amarillo)
...
void CServerDisplayer::SetWindowName() // OK
{
	...
	wsprintf(buff, "[%s] %s (Onlines : %d)", HACKSERVER_VERSION_DUMMY, HACKSERVER_CLIENT, GetUserCount());
...
void CServerDisplayer::PaintAllInfo() // OK
{
	...
	if (state == 0)
	{
		SetTextColor(hdc, RGB(200, 200, 200));  //Color de fuente modo de espera
		FillRect(hdc, &rect, this->m_brush[0]);
		TextOut(hdc, 220, 50, this->m_DisplayerText[0], strlen(this->m_DisplayerText[0])); //Espacio para centrar texto
	}
	else
	{
		SetTextColor(hdc, RGB(250, 250, 250)); //Color de fuente modo activo
		FillRect(hdc, &rect, this->m_brush[1]);
		TextOut(hdc, 250, 50, this->m_DisplayerText[1], strlen(this->m_DisplayerText[1])); //Espacio para centrar texto
	}
...
void CServerDisplayer::LogTextPaint() // OK
{
	...
	for (int n = 0;n < MAX_LOG_TEXT_LINE;n++)
	{
		...
		case LOG_ORANGE:
			...
			break;
		case LOG_PURPLE:
			SetTextColor(hdc, RGB(243, 40, 247));
			break;
		}
...
void CServerDisplayer::PaintName() // OK
{
	...
	SetTextColor(hdc, RGB(0, 0, 0)); // Color de fuente franja principal game-shield
	FillRect(hdc, &rect, this->m_brush[2]);
	TextOut(hdc, 100, 0, HACKSERVER_CLIENT, sizeof(HACKSERVER_CLIENT)); //Espacio para centrar texto
