...
void HackServerProtocolCore(int index,BYTE head,BYTE* lpMsg,int size) // OK
{
	...
	switch(head)
	{
		...
		case 0x11:
			...
			break;
		case 0x12:
			gProcessListInfo.RecvProcessList((PMSG_PROCESS_INFO_DATA_SEND*)lpMsg, index);
			break;
		case 0x13:
			gScreenShot.RecvInfo((PMSG_CLIENT_SCREEN_SHOT_SEND*)lpMsg, index);
			break;
		case 0x14:
			LogAdd(LOG_RED,"RECV Screenshot");
			gScreenShot.RecvInfo((PMSG_SERVER_SCREEN_SHOT_RECV*)lpMsg, index);
			break;
		// Example to add a new packet header option
		//case 0x12:
...
void CHClientInfoRecv(SDHP_CLIENT_INFO_RECV* lpMsg,int index) // OK
{
	...
	if(strcmp(HACKSERVER_VERSION, lpMsg->HackVersion)!=0 )
	{
		pMsg.result = 2;
		gSocketManager.DataSend(index,(BYTE*)&pMsg,pMsg.header.size);

		LogAdd(LOG_RED, "[AntiExploid] Closing detected Exploid");
		HCClientDisconnectSend(index, 2);
		gClientManager[index].m_state = CLIENT_OFFLINE;
		return;
	}
	...
	{
		LogAdd(LOG_RED, "[MultiSession] Hardware id was previously detected under suspictious activities");
		pMsg.result = 3;
		gSocketManager.DataSend(index,(BYTE*)&pMsg,pMsg.header.size);
		return;
	}

	if (gHidManager.CheckHardwareId2(lpMsg->HardwareId) == 0)
	{
		LogAdd(LOG_RED, "[MultiSessions] Closing detected multi sessions");
		HCClientDisconnectSend(index, 2);
		gClientManager[index].m_state = CLIENT_OFFLINE;
	}
	else {
		LogAdd(LOG_BLUE, "[MultiSessions %s]", lpMsg->HardwareId);
		gHidManager.InsertHardwareId2(lpMsg->HardwareId);
	}
	
	if (gBlackList.CheckHardwareId(lpMsg->HardwareId) == 0)
	{
		LogAdd(LOG_BLUE, "[gBlackList.CheckHardwareId]");
		pMsg.result = 4;
		gSocketManager.DataSend(index, (BYTE*)&pMsg, pMsg.header.size);
		return;
	}
	...
	{
		LogAdd(LOG_BLUE, "[gBlackList.CheckIpAddress]");
		pMsg.result = 5;
		gSocketManager.DataSend(index,(BYTE*)&pMsg,pMsg.header.size);
		return;
	}
	...
	if(lpMsg->IsReconnect == 0)
	{
		...
		pMsg.MacroProtectS6 = gServerInfo.m_MacroProtectS6Switch;
		...
		pMsg.LauncherType = gServerInfo.LauncherType;

		memcpy(pMsg.WindowsNameLauncher, gServerInfo.WindowsNameLauncher, sizeof(pMsg.WindowsNameLauncher));

		memcpy(pMsg.WindowsCRCLauncher, gServerInfo.WindowsCRCLauncher, sizeof(pMsg.WindowsCRCLauncher));

		memcpy(pMsg.NameLauncher, gServerInfo.NameLauncher, sizeof(pMsg.NameLauncher));
#if TESTNAMECRC
		memcpy(pMsg.ClientName,gServerInfo.ClientName,sizeof(pMsg.ClientName));

		memcpy(pMsg.ClientNameCRC,gServerInfo.ClientNameCRC,sizeof(pMsg.ClientNameCRC));
#endif
		gSocketManager.DataSend(index,(BYTE*)&pMsg,pMsg.header.size);
...
{
	char buff[128] = {0};
	char cambiocaptionname[64] = "--";
	char cambioprocessname[64] = "Main";
	switch(lpMsg->type)
	{
		...
		case CLIENT_DISCONNECT_MEMORY_DETECTION:
			gHidManager.InsertHardwareId(gClientManager[index].m_HardwareId);
			memcpy(lpMsg->CaptionName,cambiocaptionname,sizeof(cambiocaptionname));
			memcpy(lpMsg->ProcessName,cambioprocessname,sizeof(cambioprocessname));
			wsprintf(buff,"MEMORYCHANGE_DETECTION");
			break;
		...
		case CLIENT_DISCONNECT_DEBUGGER_DETECTION:
			...
			break;
		case CLIENT_DISCONNECT_VALKYRIA_DETECTION:
			gHidManager.InsertHardwareId(gClientManager[index].m_HardwareId);
			wsprintf(buff, "VALKYRIA_DETECTION");
			LogAdd(LOG_RED, "[HackDetected] Valkyria detected at ip %s", gClientManager[index].m_IpAddr);
			break;
		default:
			gHidManager.InsertHardwareId(gClientManager[index].m_HardwareId);
	...
	gHidManager.ClearHardwareId2(gClientManager[index].m_HardwareId);
}
