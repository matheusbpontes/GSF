...
#if TESTNAMECRC
void CProtect::CheckClientName(char* namex,char* namecrc) // OK
{
	if(strcmp(namex, "0") == 0)
	{
		return;
	...
	if(_stricmp(ConvertModuleFileName(name),namex) != 0)
	{
		gLog.Output(LOG_DEBUG,GetEncryptedString(34));
		SafeExitProcess();
	}

	if(strcmp(namecrc, "0") != 0){
	char tes2[32];
	wsprintf(tes2, "%d",GetCheckSumFromMain (namex));
	
	if(strcmp(namecrc, tes2) != 0)
	{
		gLog.Output(LOG_DEBUG,GetEncryptedString(35));
		SafeExitProcess();
	}
