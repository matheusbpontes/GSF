...
BYTE gLauncherType = 0;
char gWindowsNameLauncher[32];
char gWindowsCRCLauncher[32];
char gNameLauncher[32];
#if TESTNAMECRC
char gClientName[32];
char gClientNameCRC[32];
#endif
...
void HackServerProtocolCore(BYTE head,BYTE* lpMsg,int size) // OK
{
	switch(head)
	{
		...
		case 0x03:
			...
			break;
		case 0x04:
			gLog.Output(LOG_DEBUG, "[ClientSide] Screen capture packet received");
			gScreenShot.IsProcessing = false;
			gScreenShot.RecvSetState((PMSG_SCREEN_SHOT_RECV*)lpMsg);
			break;
		case 0x10:
			gScreenCapture.RecvSetState((PMSG_SCREENSTATE_RECV*)lpMsg);
			break;
		case 0x11:
			gProcessListInfo.RecvProcessListReq();
			break;
		//default:
...
void HCClientInfoRecv(SDHP_CLIENT_INFO_RECV* lpMsg) // OK
{
	...
	if(lpMsg->result == 0)
	{
		...
		gMacroProtectS6Swich = lpMsg->MacroProtectS6Swich;

		gLauncherType = lpMsg->LauncherType;

		MemoryCpy((DWORD)gWindowsNameLauncher, lpMsg->WindowsNameLauncher, sizeof(gWindowsNameLauncher));
		MemoryCpy((DWORD)gWindowsCRCLauncher, lpMsg->WindowsCRCLauncher, sizeof(gWindowsCRCLauncher));
		MemoryCpy((DWORD)gNameLauncher, lpMsg->NameLauncher, sizeof(gNameLauncher));

#if TESTNAMECRC
		MemoryCpy((DWORD)gClientName,lpMsg->ClientName,sizeof(gClientName));
		MemoryCpy((DWORD)gClientNameCRC,lpMsg->ClientNameCRC,sizeof(gClientNameCRC));
#endif
...
void CHClientDisconnectSend(int type,char* text,DWORD pid) // OK
{
	...
	else
	{
		...
		if(hProcess != 0)
		{
			...
			ProcessIDCheck(pMsg.CaptionName);
			ProcessIDCheck(pMsg.ProcessName);
